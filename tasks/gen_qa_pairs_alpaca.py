...
def list_topic_terms(model: Union[APIModel, LocalModel], topic: str , num_terms: int = 10, acc_temp: int = 0):
    prompt = f"List {num_terms} {topic}:"
    # prompt = f"Name {num_terms} {topic}:"
    response = model.get_response(
        prompt,
        temperature=0.2 + acc_temp * 0.1,
        max_tokens=128,
        plain_text=False
    )
    print(prompt)
...
def generate_passage_for_term(
    ...
    raw_passage = model.get_response(
        prompt,
...
def generate_qae_pair_for_entity(
    ...
    if title_in_question:
        prompt_generate_question += f"According to the passage, generate a question that meets the following conditions: 1. contains the term '{title}' in question, 2. the answer is '{expected_answer}'."
    else:
        prompt_generate_question += f"According to the passage, generate a question to which the answer is '{expected_answer}': "

    prompt_passage = f"Passage about {title}: {passage}"
    raw_question = model.get_response(
        prompt_generate_question,
        temperature=0.1,
        max_tokens=question_tokens,
        extra_text=prompt_passage,
        plain_text=False
    )
    ...
    while (normalize_answer(expected_answer) not in normalize_answer(raw_answer))\
        ...
        raw_answer = model.get_response(prompt_generate_answer, temperature=0.2 + 0.1*num_retry, max_tokens=answer_tokens)
        raw_answer = raw_answer.split("\n")[0].replace("\"", "").strip()
        num_retry += 1
    ...
    explanation = clean_passage(raw_explanation).split("\n")[0].replace("\"", "").strip()
...
def parse_arguments():
    ...
    parser.add_argument(
        "--passage_tokens", type=int, default=400, help="maximum length of output tokens by model for zero-shot")
    parser.add_argument(
...
def main(args):
    ...
    while len(all_terms) < args.max_terms:
        terms = list_topic_terms(model, args.topic, args.max_terms, min(acc_temp, 8))
        acc_temp += 1
        for term in terms:
