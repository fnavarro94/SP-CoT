    ...
    while True:
        ...
        if response == "RESPONSE_ERROR":
            output_sample = {
                ...
                "pred_answer": input_sample["pred_answer"],
                "response": response,
            }
        else:
            ...
            output_sample = {
                "question": input_sample["question"],
                "answer": input_sample["answer"],
                "pred_answer": pred_answers + input_sample["pred_answer"],
                "prompt": prompt,
                "response": response,
            }
            if do_eval:
        ...
        if passage == "RESPONSE_ERROR":
            ...
            if raw_pred == "RESPONSE_ERROR":
                ...
                output_sample = {
                    "question": input_sample["question"],
                    "answer": input_sample["answer"],
                    "pred_answer": pred_answers + input_sample["in"],
                    "prompt": answer_prompt,
                    "response": raw_pred,
                    "passage": passage,
                }
...
async def self_prompt_cot_worker(
    ...
    while True:
        ...
        response = await get_chat_response(model=model, prompt=prompt, max_tokens=max_tokens, temperature=1)
...
def parse_arguments():
    ...
    parser.add_argument("--method", type=str, default="genread",
                        choices=["zeroshot", "zeroshot-cot", "genread", "self-prompt", "auto-cot", "self-prompt-cot", "manual-cot"],
                        help="method name")
...
async def main(args: argparse.Namespace):
    ...
    if args.flag != "":
        output_jsonl_path = os.path.join(output_dir, f"{args.method}-{args.flag}-sc.jsonl")
    else:
        output_jsonl_path = os.path.join(output_dir, f"{args.method}-sc.jsonl")
    ...
    if args.use_cache:
        ...
        if os.path.exists(output_jsonl_path.replace(".jsonl", ".json")):
            with open(output_jsonl_path.replace(".jsonl", ".json"), "r") as f:
                dataset = json.load(f)
            for i in range(len(dataset)):
                for sample in raw_dataset:
                    if sample["question"] == dataset[i]["question"]:
                        dataset[i]["demos"] = sample["demos"]
                        break
        else:
            dataset = raw_dataset
        existing_dataset = []
    ...
    for data in dataset:
        ...
        existing_answers = data["pred_answer"] if "pred_answer" in data else []

        input_sample = {
            "question": question,
            "answer": gold_answers,
            "pred_answer": existing_answers,
            "total": total
        }
