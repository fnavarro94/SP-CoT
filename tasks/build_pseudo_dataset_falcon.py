...
from src.models import Falcon, LocalModel
...
def parse_argument():
    ...
    parser.add_argument("--data_dir", type=str, default="../data/self-prompt-cot/falcon-7b/",
    ...
    if args.output_composition_path is None:
        args.output_composition_path = os.path.join(args.output_dir, "all_datasets_falcon.pkl")

    if args.output_filtered_path is None:
        args.output_filtered_path = os.path.join(args.output_dir, "pseudo_dataset_raw_falcon.json")

    if args.output_completion_path is None:
        args.output_completion_path = os.path.join(args.output_dir, "pseudo_dataset_complete_falcon.json")

    if args.output_yesno_path is None:
        args.output_yesno_path = os.path.join(args.output_dir, "pseudo_dataset_yesno_falcon.json")

    if args.output_merge_path is None:
        args.output_merge_path = os.path.join(args.output_dir, "pseudo_dataset_falcon.json")
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_filtered_path) and not args.overwrite_output:
        ...
        max_duplicate_degrees = [0, 2, 4, 4, 4, 4]
        filtered_datasets = {}
    ...
    if not os.path.exists(args.output_completion_path) or args.overwrite_output:
        ...
        model = Falcon("falcon-7b")
        # from src.api import ChatGPT
        # model = ChatGPT("gpt-3.5-turbo-0301")
    ...
    # if not os.path.exists(args.output_merge_path):
    ...
    for hop_type in filtered_datasets:
        ...
        for sample in raw_dataset:
            # if sample["question"] == "":
            #     continue
            # if "[" in sample["question"] or "]" in sample["question"]:
            #     continue
            # if "\n" in sample["question"]:
            #     continue
            # if "<" in sample["question"] or ">" in sample["question"]:
            ...
            #     continue
            # if sample["hops"][-1]["answer"].lower() in sample["hops"][0]["question"].lower():
            #     continue
            # if sample["answer"].lower() in sample["question"].lower():
            #     continue
            # if len(sample["question"].split()) < 5:
            #     continue

            sample["question"] = sample["question"].replace("\"", "").replace("\'", "").strip()
            for i in range(len(sample["hops"])):
                sample["hops"][i]["question"] = sample["hops"][i]["question"].replace("\"", "").replace("\'", "").strip()
                sample["hops"][i]["evidence"] = sample["hops"][i]["evidence"].split(". ")[0] + "."
        ...
        print(f"Length of {hop_type} merged dataset: {raw_length}")
        print(f"Length of {hop_type} original dataset: {new_length}")
