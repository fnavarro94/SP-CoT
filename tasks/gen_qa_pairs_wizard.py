...
def generate_qae_pair_for_entity(
    ...
    if title_in_question:
        ...
        prompt_generate_question += f"Passage about {title}: {passage}\n" \
                                    f"'{expected_answer}' is the answer to which question:"
        if ban_entity_list:
    ...
    # print(f"[{print_now(1)}] Generated question: [{question}]")
    ...
    prompt_generate_answer += f"Passage about {title}: {passage}\n\n" \
                              f"Question: {question}\n\n" \
                              f"Answer in just one entity within 6 words: "
    raw_answer = model.get_response(
        prompt_generate_answer,
        temperature=0.1,
    ...
    prompt_generate_explanation += f"Passage about {title}: {passage}\n\nQuestion: {question}\n\nAnswer: {answer}\n\n" \
                                   "Extract an evidence sentence from passage as explanation: "

    raw_explanation = model.get_response(
        prompt_generate_explanation,
        temperature=0.1,
    ...
    raw_explanation = raw_explanation.replace("*", "").replace("\"", "").strip()
    explanation = clean_passage(raw_explanation).replace("\n", " ").strip()
...
def generate_multi_hop_quadruples(
    ...
    for entity in entities[:num_entity_per_passage]:
...
def main(args):
    ...
    if args.model_name in [
        ...
        "alpaca-7b", "alpaca-13b", "llama-13b", "vicuna-13b"
    ]:
