...
def list_topic_terms(model: Union[APIModel, LocalModel], topic: str , num_terms: int = 10):
    ...
    response = model.get_response(prompt, temperature=0.5, max_tokens=128)
    print(response)
    if topic == "songs":
        # Extract the entity in brackets, e.g. "Bad Moon Rising" from:
        # 1. "Bad Moon Rising" by Creedence Clearwater Revival
        terms = re.findall(r'\d+\. ([A-Za-z\"\' ]+)', response)
        terms = re.findall(r'"([^"]*)"', " ".join(terms))
    else:
        terms = re.findall(r'\d+\. ([A-Za-z ]+)', response)
    terms = [it.strip() for it in terms][:num_terms]
    print(f"Collected {len(terms)}, they are: {terms}")
    return terms
...
def generate_qae_pair_for_entity(
    ...
    if title_in_question:
        ...
        prompt_generate_question += f"Passage about {title}: {passage}\n" \
                                    f"'{expected_answer}' is the answer to the question: "
        if ban_entity_list:
    ...
    question = (re.split("(\?)|(\.)", raw_question)[0] + "?").replace("\n", " ").replace("\"", "").replace("\'", "").strip()
    ...
    while normalize_answer(expected_answer) in normalize_answer(question):
        ...
        question = (re.split("(\?)|(\.)", raw_question)[0] + "?").replace("\"", "").replace("\'", "").strip()
        num_retry += 1
