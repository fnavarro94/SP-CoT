...
from src.models import LocalModel, Wizard
    ...
    for data in tqdm(dataset, desc=f"Generating {hop_type} questions..."):
        ...
        for demo in demos:
            ...
            prompt += f"Raw question: {raw_question}\n" \
                      f"Replace the sentence within [] with a relative clause and make the raw question into a natural question: {target_question}\n\n"
...
def generate_yes_no_questions(
    ...
    for data in tqdm(dataset, desc=f"Generating {hop_type} yes no questions..."):
        ...
        for demo in yesno_demo:
            ...
            if not raw_question.endswith("?"):
                raw_question += "?"
            prompt += f"Reform the question to a general interrogative sentence that can be answered with {demo['reformulated_answer']}: {raw_question}\n\n"
        ...
        yesno_question = yesno_question.replace("Question:", "").strip()
...
def parse_argument():
    ...
    parser.add_argument("--data_dir", type=str, default="../data/self-prompt-cot/vicuna-13b/",
    ...
    if args.output_composition_path is None:
        args.output_composition_path = os.path.join(args.output_dir, "all_datasets_wizard.pkl")

    if args.output_filtered_path is None:
        args.output_filtered_path = os.path.join(args.output_dir, "pseudo_dataset_raw_wizard.json")

    if args.output_completion_path is None:
        args.output_completion_path = os.path.join(args.output_dir, "pseudo_dataset_complete_wizard.json")

    if args.output_yesno_path is None:
        args.output_yesno_path = os.path.join(args.output_dir, "pseudo_dataset_yesno_wizard.json")

    if args.output_merge_path is None:
        args.output_merge_path = os.path.join(args.output_dir, "pseudo_dataset_wizard.json")
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_filtered_path) and not args.overwrite_output:
        ...
        multi_hop_datasets = pickle.load(open(args.output_composition_path, "rb"))
        max_duplicate_degrees = [0, 1, 3, 2, 4, 3]
        filtered_datasets = {}
